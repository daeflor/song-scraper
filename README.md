...
You also have the option to 'scrape' the current playlist, which will cause an automatic scroll through the list so the extension can get the complete list of songs by extracting it from the DOM. It sounds janky, but I've thoroughly tested and iterated on this approach, and it's now quite a quick on robust process.
...
In the case above, two tracks were removed. Note the extension icon also shows a red "-2", so even without opening the extension you can tell if some songs have been removed. A background service worker is used to accomplish this, along with Chrome sync storage. 
